/**
 *Submitted for verification at sepolia.basescan.org on 2025-10-01
*/
...
/**
 * @title OracleToken
 * @notice Governance token for OracleMarket platform
 * @dev Implements voting, burning, and vesting mechanisms
 */
contract OracleToken is ERC20, ERC20Burnable, ERC20Votes, Ownable, Pausable {
    ...
    constructor() 
        ...
        Ownable(msg.sender)
    {
        // Initial mint will be done in phases
    }
    ...
    /**
     * @notice Mint tokens for liquidity mining and rewards
     * @dev Only owner (Treasury contract) can mint
     */
    ...
    // Override required functions for OpenZeppelin 5.x
    function _update(
        address from,
        address to,
        uint256 amount
...
contract MarketFactory is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public constant MARKET_CREATION_FEE = 100 * 10**18;
    uint256 public marketCount;
    
    enum MarketType { BINARY, CATEGORICAL, SCALAR }
    enum MarketStatus { ACTIVE, CLOSED, RESOLVED, INVALID }
    enum ResolutionSource { CHAINLINK, UMA, MANUAL, API }
    ...
    event MarketInvalidated(bytes32 indexed marketId, string reason);
    ...
    function createMarket(
        ...
        require(resolutionTime > closingTime, "Invalid resolution time");
        ...
        bytes32 marketId = keccak256(
            abi.encodePacked(question, msg.sender, block.timestamp, marketCount++)
        );
        ...
        emit MarketCreated(marketId, msg.sender, question, category, marketType, closingTime);
    ...
    function closeMarket(bytes32 marketId) external {
        Market storage market = markets[marketId];
        require(market.id != bytes32(0), "Market not found");
        require(market.status == MarketStatus.ACTIVE, "Not active");
        require(
            block.timestamp >= market.closingTime || msg.sender == owner(),
            "Not closing time"
        );
    ...
    function resolveMarket(bytes32 marketId, uint256 outcome) external onlyOwner {
        Market storage market = markets[marketId];
        require(market.id != bytes32(0), "Market not found");
    ...
    function invalidateMarket(bytes32 marketId, string memory reason) external onlyOwner {
        Market storage market = markets[marketId];
        require(market.id != bytes32(0), "Market not found");
