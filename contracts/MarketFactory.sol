 ...
 * @dev Production-ready version with proper initialization and simplified interface
 */
contract MarketFactory is Ownable, ReentrancyGuard, Pausable {
    IOracleToken public oracleToken;
    IHybridAMM public hybridAMM;
    address public oracleResolver;

    uint256 public constant MARKET_CREATION_FEE = 10 * 10**18; // 10 ORACLE tokens
    uint256 public constant MIN_INITIAL_LIQUIDITY = 100 * 10**18; // 100 ORACLE tokens minimum
    ...
    event Initialized(address oracleToken, address hybridAMM, address oracleResolver);
     ...
     * @param _hybridAMM Address of the HybridAMM contract
    ...
    function initialize(
        ...
        require(_hybridAMM != address(0), "Invalid hybrid AMM");
        require(_oracleResolver != address(0), "Invalid oracle resolver");

        oracleToken = IOracleToken(_oracleToken);
        hybridAMM = IHybridAMM(_hybridAMM);
        oracleResolver = _oracleResolver;
        initialized = true;

        emit Initialized(_oracleToken, _hybridAMM, _oracleResolver);
    }

    /**
     * @notice Create a new prediction market
     * @dev Requires approval of (MARKET_CREATION_FEE + initialLiquidity) ORACLE tokens
     ...
     * @param initialLiquidity Amount of ORACLE tokens to seed the market pool
    ...
    function createMarket(
        ...
        // Transfer initial liquidity to HybridAMM
        require(
            oracleToken.transferFrom(msg.sender, address(hybridAMM), initialLiquidity),
        ...
        market.liquidity = initialLiquidity;

        creatorMarkets[msg.sender].push(marketId);
        allMarketIds.push(marketId);

        // Initialize AMM pool
        hybridAMM.createMarketPool(marketId, initialLiquidity);
